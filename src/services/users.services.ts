...
import { RegisterReqBody, UpdateMeRequestBody } from '~/models/requests/User.request'
...
class UsersService {
  ...
  async register(payload: RegisterReqBody) {
    ...
    const email_verify_token = await this.signEmailVerifyToken({
      ...
      verify: UserVerifyStatus.Unverified //Trạng thái chưa verify
    })
  ...
  async resendVerifyEmail(user_id: string) {
    const email_verify_token = await this.signEmailVerifyToken({
      user_id,
      verify: UserVerifyStatus.Unverified // Chưa verify email
    })
    // Gỉa bộ gửi email
